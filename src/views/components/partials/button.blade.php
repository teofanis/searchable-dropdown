<button x-ref="dropdown-button" @click.prevent="toggleListboxVisibility()" :aria-expanded="open" aria-haspopup="listbox" 
    ...
    <input id="{{$dropdownFunctionName}}_button" x-ref="search" x-show="open" x-model.debounce.250ms="search" @keydown.enter.stop.prevent="selectOption()" type="search" placeholder="{{$searchFieldPlaceholder}}" class="w-full h-full form-control focus:outline-none pr-2" />
